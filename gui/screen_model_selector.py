...
class ModelSelectorScreen(ctk.CTkFrame):
    ...
    def populate_models(self):
        for m in MODELS:
            ...
            cb = ctk.CTkCheckBox(card, text=f"{m['id']} - {m['desc']}", variable=var, command=toggle, font=FONT_MAIN, text_color=TEXT_COLOR, fg_color=ACCENT_COLOR)
    ...
    def _do_pull(self):
        ...
        if not installed:
            ...
            def log_install(msg): self.status_lbl.configure(text=f"Installing Ollama: {msg[:50]}...", text_color=ACCENT_COLOR)
            try:
                from platforms.common.ollama_universal import setup_ollama
                from pathlib import Path
                import platform
                if platform.system() == "Windows": install_dir = Path(os.environ.get("LOCALAPPDATA", str(Path.home() / "AppData" / "Local"))) / "Programs" / "Ollama"
                else: install_dir = Path.home() / ".ollama_bin"
        ...
        for model in self.selected_models:
            ...
            def log_cb(msg): self.status_lbl.configure(text=f"Pulling {model}: {msg[:50]}...", text_color=ACCENT_COLOR)
